...
public class main_activity extends Activity {
	...
	void initData() {
		...
		// 设置容器
		utils.SetView(lincontainer, this, groups.size());
		// 将容器内的容器取出
		...
		for (int row = 0; row < groups.size(); row++) {
			...
			for (int col = 0; col < group.tags.size(); col++) {
				View view = initTextView(col, row, group.tags.get(col),
						pars.get(row), clicker1);
				lin.addView(view);
			}
		}
	}
	...
	OnClickListener clicker1 = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			if(tag.isSelected)
			{
				...
				v.setBackgroundResource(R.drawable.icon_index_user_dna_n);
			}
			else 
			{
				tag.isSelected=true;
	...
	private void addtwo(LinearLayout lin, List<MyTag> tags, int colIndex,
			int rowIndex) {

		View view1 = initTextView(colIndex, rowIndex, tags.get(0),
				pars.get(rowIndex), clicker1);
		for (int i = colIndex; i < lin.getChildCount(); i++) {
			View temp_view = lin.getChildAt(i);
			if (i == colIndex)
				temp_view.setTag(String.valueOf(i + 1) + "_"
						+ String.valueOf(rowIndex));
			else
				temp_view.setTag(String.valueOf(i + 2) + "_"
						+ String.valueOf(rowIndex));
		}
		View view2 = initTextView(colIndex + 1, rowIndex, tags.get(1),
				pars.get(rowIndex), clicker1);
		lin.addView(view1, colIndex);
		lin.addView(view2, colIndex + 2);
	}

	private void addone(LinearLayout lin, List<MyTag> tags, int colIndex,
			int rowIndex) {
	...
	View initTextView(int col_index, int row_index, MyTag tag,
		...
		tv.setBackgroundResource(R.drawable.icon_index_user_dna_n);
		...
		// tv.setTag(String.valueOf(col_index) + "_" +
		// String.valueOf(row_index));
